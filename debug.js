...
const pageTitles = {
    ...
    dashboard:
    "<i class='fa-solid fa-desktop' style='margin-right:8px;'></i>Dashboard",

    proposal:
    "<i class='fa-solid fa-clipboard-list' style='margin-right:8px;'></i>Proposal",

    upload:
    "<i class='fa-solid fa-upload' style='margin-right:8px;'></i>Upload",

    review:
    "<i class='fa-solid fa-star' style='margin-right:8px;'></i>Review",
...
function showPage(name, btn){
    ...
    const title = pageTitles[name] || "<i class='fa-solid fa-cog'></i>Page";
...
function openPageFromQuery(){
    ...
    if(page && pageTitles[page]){
        ...
        if(btn) {
            ...
            // If button not found, still set title and page
            document.querySelectorAll(".page").forEach(p=>p.classList.remove("active"));
            document.querySelectorAll(".nav-item").forEach(b=>b.classList.remove("active"));
            if(document.getElementById("page-"+page)){
                document.getElementById("page-"+page).classList.add("active");
            }
            const title = pageTitles[page] || "<i class='fa-solid fa-cog'></i>Page";
            document.getElementById("pageTitle").innerHTML = title;
        }
    }
}

document.addEventListener('DOMContentLoaded', openPageFromQuery);
                ...
                ${
                    ...
                    ? `
                        <div class="file-edit-box">
                            ...
                            <label for="editFile" class="change-file-btn">
...
function updateDashboardStats() {
    ...
    const pending = researches.filter(r => r.status === "pending").length;
    ...
    if(cards.length >= 3){
        ...
        cards[0].querySelector("p").innerText = total + " projects ";

        cards[1].querySelector("p").innerText = pending + " pending ";

        cards[2].querySelector("p").innerText = approved + " rejected ";
    }
    ...
    function updateChartData() {
        ...
        // 👉 update dataset
        myChart.data.datasets[0].data = [approved, pending, rejected];
...
function updateCenterText() {
    ...
    myChart.options.plugins[0].text = percent + "%";
}
...
function deleteResearch(id){
    ...
    fetch("delete.php", {
        method: "POST",
        body: formData,
        credentials: "logindetails.php" // 🔥 session send වෙනව ensure කරන්න
    })
    .then(res => res.text())
    .then(data => {
        ...
        if(data.trim() === "success"){
            ...
            // UI එකෙන් remove කරන්න (no reload)
...
function updateLegend() {
    ...
    document.getElementById("legendCompleted").innerText = `Completed ${approvedPct}%`;
    document.getElementById("legendPending").innerText   = `Pending ${pendingPct}%`;
    document.getElementById("legendRejected").innerText  = `Rejected ${rejectedPct}%`;
}
...
const myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            data: [0, 0, 0], // මෙතනට ඔයාගේ dynamic values දාන්න
            backgroundColor: [
                '#28a745', // Green
                '#ffc107', // Yellow
                '#dc3545'  // Red
    ...
    plugins: [{
        ...
        beforeDraw: function(chart) {
            ...
            const percent = total ? Math.round((pending / total) * 100) : 0;
